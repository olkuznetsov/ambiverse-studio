...
It drives the [Ambiverse](https://www.youtube.com/@ambiverseworlds) ambient-music pipeline: a Python system that turns AI-generated images, Veo clips, and Suno music into 2-hour immersive videos — Ken Burns stills, DepthFlow 2.5D parallax, or Veo "living-world" slow-motion (dedupe → Real-ESRGAN super-resolution → RIFE optical-flow interpolation) — then auto-generates metadata, translations, and thumbnails, and publishes to YouTube with staggered scheduled Shorts. Studio wraps that pipeline — it never reimplements it — and adds the things a pile of scripts can't give you: a live view of pipeline state, one-button end-to-end builds with real progress bars (including the multi-GB upload), media browsing, housekeeping, and analytics that track the channel's actual open questions.
...
| **Assets** | Per-theme image grid with display-time warnings (aspect ratio, dupes), in-browser music playback, an outputs browser with `<video>` preview over HTTP range requests, and a **housekeeping card** that finds reclaimable gigabytes with explicit per-file reasons (superseded pre-upscale clips, silent pre-mux builds). Safe actions: trash → recoverable, set-as-thumb. |
| **Build** | Wizards that assemble the env + command, **show the exact command that will run**, run an inline preflight, then queue it: a main build (Ken Burns / DepthFlow, every knob), a Shorts builder (image or Veo-clip source), and the Veo flow — enhance batch, clip-picker assembly, or **one-button full auto**: enhance everything → shuffled 1–8 h video + library music bed → optionally publish (metadata, scheduled upload, staggered Shorts). A disk guard refuses to queue a build the disk can't hold. |
| **Jobs** | SQLite-backed queue, live log streaming (SSE), per-phase progress bars — ESRGAN frames, render clips, reel assembly, even **upload chunks** — and **cancel that kills the whole process group** (ffmpeg/RIFE children included). Single-flight so heavy renders never overlap; desktop notification when a long job lands. |
| **Channel** | YouTube analytics that answer a question, not just dump numbers: YPP progress bars, a **format A/B card** (Veo living-world vs image-based — retention/AVD side by side, sourced from build manifests), a **per-world retention rollup** ("what should I make next"), growth charts with upload-event markers, traffic/geo/device breakdowns, scheduled-publish list, and a manual impressions/CTR log for the one metric no API exposes. |
...
- **Attribution is evidence-ranked, not guessed.** Each upload's theme and format (Veo vs image) carry a source tag — build-manifest video ids first, the pipeline's own run logs next, keyword inference last — and the UI marks inferred values, so the rollups stay honest about their confidence.
